                                ...
                                @foreach ($question->options as $option)    
                                    <div class="flex items-center mb-4">
                                        <input type="radio" name="i_{{ $no_question }}" id="i_{{ $no_question }}_{{ $no_option }}" value="{{ $option->value }}" class="w-4 h-4 flex-shrink-0"
                                            {{ ($question->is_required == '1') ? 'required' : '' }} 
                                            {{ ($question->pivot->value == $option->value) ? 'checked' : 'disabled' }}
                                        >
                                        @php
                            ...
                            <div id="i_{{ $no_question }}" class="mt-3">
                                <div class="flex items-center mb-4">
                                    <input type="radio" name="i_{{ $no_question }}" id="i_{{ $no_question }}_1" value="Yes" class="w-4 h-4 flex-shrink-0"
                                        {{ ($question->is_required == '1') ? 'required' : '' }} 
                                        {{ ($question->pivot->value == 'Yes') ? 'checked' : 'disabled' }}
                                    >
                                ...
                                <div class="flex items-center mb-4">
                                    <input type="radio" name="i_{{ $no_question }}" id="i_{{ $no_question }}_2" value="No" class="w-4 h-4 flex-shrink-0"
                                        {{ ($question->is_required == '1') ? 'required' : '' }} 
                                        {{ ($question->pivot->value == 'No') ? 'checked' : 'disabled' }}
                                    >
                                ...
                                @for ($i = 1; $i <= $option_number; $i++)
                                    ...
                                    <div>
                                        <input type="radio" name="sr_{{ $no_question }}" id="{{ $i }}" class="hidden" value="{{ $i }}"
                                            {{ ($question->is_required == '1') ? 'required' : '' }} 
                                            {{ ($question->pivot->value == $i) ? 'checked' : 'disabled' }}
                                        >
                                        <label for="{{ $i }}">
                                            <div class="w-9 h-9 mx-1 mb-3 rounded-md flex justify-center items-center border-2 border-dark border-opacity-30 hover:bg-dark hover:text-white {{ ($question->pivot->value == $i) ? 'selected-selection' : 'unselected-selection' }}">
                                                {{ $i }}
                                            </div>
                                        </label>
                                    </div> 
            ...
            @if ($questions_f->count() > 0)
                ...
                @foreach ($questions_f as $question)
                    ...
                    <div id="f_{{ $no_question }}_div" class="w-full pb-7 pt-5 {{ ($question->no == 1) ? '' : 'border-t-2' }}">
                        ...
                        <textarea name="f_{{ $no_question }}" id="f_{{ $no_question }}" rows="3" class="text-sm border-2 border-slate-300 rounded-md w-full p-2.5 focus:border-secondary focus:outline-none" {{ ($question->is_required == '1') ? 'required' : '' }}>{{ $question->pivot->value }}</textarea>
                ...
                @foreach ($questions_o as $question)
                    ...
                    @elseif($question->input_type == '9')
                        ...
                        <div id="o_{{ $no_question }}_div" class="w-full pb-7 pt-5 {{ ($question->no == 1) ? '' : 'border-t-2' }}">
                            ...
                            <textarea name="o_{{ $no_question }}" id="o_{{ $no_question }}" rows="3" class="text-sm border-2 border-slate-300 rounded-md w-full p-2.5 focus:border-secondary focus:outline-none" {{ ($question->is_required == '1') ? 'required' : '' }}>{{ $question->pivot->value }}</textarea>
