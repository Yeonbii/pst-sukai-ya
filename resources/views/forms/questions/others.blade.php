                ...
                @foreach ($questions_o as $question)
                    ...
                    @elseif($question->input_type == '9')
                        ...
                        <div id="o_{{ $no_question }}_div" class="w-full pb-7 pt-5 {{ ($question->no == 1) ? '' : 'border-t-2' }}">
                            ...
                            {{-- Input Answer Start --}}
                            <textarea name="o_{{ $no_question }}" id="o_{{ $no_question }}" rows="3" class="text-sm border-2 border-slate-300 rounded-md w-full p-2.5 focus:border-secondary focus:outline-none" {{ ($question->is_required == '1') ? 'required' : '' }} >@if (session()->has('form_o')){{ $form_o['o_'.$no_question] }}@endif</textarea>
                            {{-- Input Asnswer End --}}
